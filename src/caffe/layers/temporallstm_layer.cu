...
void TemporalLstmLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int t = 0; t < T_; ++t) {
    ...
    caffe_gpu_sigmoid_forward(3*H_, pre_i_t, i_t);
    caffe_gpu_tanh_forward(H_, pre_g_t, g_t);
    ...
    caffe_gpu_tanh_forward(H_, c_t, tanh_c_t);
...
template <typename Dtype>
void TemporalLstmLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    ...
    const vector<Blob<Dtype>*>& bottom) {
      ...
      for (int t = T_-1; t >= 0; --t) {
        ...
        caffe_gpu_tanh_backward(H_, tanh_c_t, dc_t, dc_t);
        if (t < T_-1) {
        ...
        caffe_gpu_sigmoid_backward(3*H_, i_t, di_t, pre_di_t);
        caffe_gpu_tanh_backward(H_, g_t, dg_t, pre_dg_t);
...
INSTANTIATE_LAYER_GPU_FUNCS(TemporalLstmLayer);
