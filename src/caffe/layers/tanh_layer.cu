...
/*
template <typename Dtype>
void TanHLayer<Dtype>::Forward_gpu(int N, const Dtype* bottom, Dtype* top) {
  TanHForward<Dtype><<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(
      N, bottom, top);
  CUDA_POST_KERNEL_CHECK;
}
template <typename Dtype>
void TanHLayer<Dtype>::Backward_gpu(int N, const Dtype* top_data,const Dtype* top_diff, Dtype* bottom_diff) {
