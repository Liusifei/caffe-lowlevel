...
void TemporalLstmLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
      const int num_output = this->layer_param_.temporallstm_param().num_output();
      bias_term_ = this->layer_param_.temporallstm_param().bias_term();
      //clipping_threshold_ = this->layer_param_.temporallstm_param().clipping_threshold();
      ...
      if (this->blobs_.size() > 0) {
        ...
        shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
            this->layer_param_.temporallstm_param().weight_filler()));
        ...
        if (bias_term_) {
          ...
          shared_ptr<Filler<Dtype> > bias_filler(GetFiller<Dtype>(
              this->layer_param_.temporallstm_param().bias_filler()));
...
template <typename Dtype>
void TemporalLstmLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
      ...
      //this is different from original code, just for avoiding further change in layer.hpp
      //now Sequence length is equal to batch size, we can change it IF NECESSARY.
      this->PreStartSequence();
}

template <typename Dtype>
void TemporalLstmLayer<Dtype>::PreStartSequence() {
...
template <typename Dtype>
void TemporalLstmLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
    //LOG(ERROR) << "Forward";
  ...
  for (int t = 0; t < T_; ++t) {
    ...
    caffe_cpu_sigmoid_forward(3*H_, pre_i_t, i_t);
    caffe_cpu_tanh_forward(H_, pre_g_t, g_t);
    ...
    caffe_cpu_tanh_forward(H_, c_t, tanh_c_t);
...
template <typename Dtype>
void TemporalLstmLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int t = T_-1; t >= 0; --t) {
    ...
    caffe_cpu_tanh_backward(H_, tanh_c_t, dc_t, dc_t);
    if (t < T_-1) {
    ...
    caffe_cpu_sigmoid_backward(3*H_, i_t, di_t, pre_di_t);
    caffe_cpu_tanh_backward(H_, g_t, dg_t, pre_dg_t);
...
#ifdef CPU_ONLY
STUB_GPU(TemporalLstmLayer);
#endif

INSTANTIATE_CLASS(TemporalLstmLayer);
REGISTER_LAYER_CLASS(TemporalLstm);
