...
#include "caffe/util/ImgAffineWarp.hpp" // for image data similarity perturbation
...
bool Read_Images_ToDatum(const string& filenames,vector< int> channelNum,
            ...
            bool is_grid_perturb,float move_x,float move_y, bool flipimg,vector<int> resizetype)//add by liangji, 20150226
{
    ...
    for(int data_id = 0;data_id<imagenames.size();data_id++)
    {
        ...
        //added by zhufuguo@2015-1-22
        //perturb and resize the image
        {
            ...
            float scale_local = float(cv_img_origin.cols)*scale / float(sz.width);//scale
            
            float alpha = angle*CV_PI/180; //rotation in radian

            cv::Mat_<double> M = Get_Affine_matrix(srcCenter,dstCenter,alpha,scale_local);
            mAffineWarp(M,cv_img_origin,cv_img,resizetype[data_id] ? INTER_NEAREST : INTER_LINEAR);
        }
