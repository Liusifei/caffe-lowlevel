...
Dtype Blob<Dtype>::min_data() const{
  ...
  Dtype min_dt = data_at(0,0,0,0);
  for (int n = 0; n < num_; ++n) {
    for (int c = 0; c < channels_; ++c) {
      for (int h = 0; h < height_; ++h) {
        for (int w = 0; w < width_; ++w) {
          if ( min_dt > data_at(n, c, h, w) ) min_dt = data_at(n, c, h, w);
        }
      }
    }
  }
...
Dtype Blob<Dtype>::min_diff() const{
    int num_ = shape_[0],channels_=shape_[1],height_=shape_[2],width_=shape_[3]; 
  Dtype min_df = diff_at(0, 0, 0, 0);
  for (int n = 0; n < num_; ++n) {
    for (int c = 0; c < channels_; ++c) {
      for (int h = 0; h < height_; ++h) {
        for (int w = 0; w < width_; ++w) {
          if ( min_df > diff_at(n, c, h, w) ) min_df = diff_at(n, c, h, w);
        }
      }
    }
  }
...
Dtype Blob<Dtype>::max_data() const{
    int num_ = shape_[0],channels_=shape_[1],height_=shape_[2],width_=shape_[3]; 
  Dtype max_dt = data_at(0,0,0,0);
  for (int n = 0; n < num_; ++n) {
    for (int c = 0; c < channels_; ++c) {
      for (int h = 0; h < height_; ++h) {
        for (int w = 0; w < width_; ++w) {
          if ( max_dt < data_at(n, c, h, w) ) max_dt = data_at(n, c, h, w);
        }
      }
    }
  }
...
Dtype Blob<Dtype>::max_diff() const{
    int num_ = shape_[0],channels_=shape_[1],height_=shape_[2],width_=shape_[3]; 
  Dtype max_df = diff_at(0,0,0,0);
  for (int n = 0; n < num_; ++n) {
    for (int c = 0; c < channels_; ++c) {
      for (int h = 0; h < height_; ++h) {
        for (int w = 0; w < width_; ++w) {
          if ( max_df < diff_at(n, c, h, w) ) max_df = diff_at(n, c, h, w);
        }
      }
    }
  }
