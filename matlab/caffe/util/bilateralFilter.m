...
if ~exist( 'edgeMin', 'var' ) || isempty(edgeMin),
    edgeMin = min( edge( : ) );
    warning( 'edgeMin not set!  Defaulting to: %f\n', edgeMin );
end

if ~exist( 'edgeMax', 'var' )|| isempty(edgeMax),
    edgeMax = max( edge( : ) );
...
if ~exist( 'sigmaSpatial', 'var' )|| isempty(sigmaSpatial),
    sigmaSpatial = min( inputWidth, inputHeight ) / 16;
    warning( 'Using default sigmaSpatial of: %f\n', sigmaSpatial );
end

if ~exist( 'sigmaRange', 'var' )|| isempty(sigmaRange),
    sigmaRange = 0.1 * edgeDelta;
    fprintf( 'Using default sigmaRange of: %f\n', sigmaRange );
end

if ~exist( 'samplingSpatial', 'var' )|| isempty(samplingSpatial),
    samplingSpatial = sigmaSpatial;
end
