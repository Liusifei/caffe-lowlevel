...
class EyeFiller : public Filler<Dtype> {
 public:
  ...
  virtual void Fill(Blob<Dtype>* blob) {
    CHECK(blob->count());
    int num = this->filler_param_.filter_num();
    int channel = this->filler_param_.filter_channel();
    int width = this->filler_param_.filter_width();
    int height = this->filler_param_.filter_height();
    
    CHECK(blob->count() == num*channel*width*height);
    Dtype * data = blob->mutable_cpu_data();
    int dim = channel*height*width;
    int spatialdim = height*width;
    for(int n=0;n< num;n++)
    {
        for(int c=0;c<channel;c++)
        {
            for(int h=0;h<height;h++)
            {
                ...
                {
                    if(w == h)
                       data[n*dim + c*spatialdim + h*width + w] = 1;
                    else
                       data[n*dim + c*spatialdim + h*width + w] = 0;
                }
            }
        }
    }
