...
class DropoutLayer : public NeuronLayer<Dtype> {
 ...
 protected:
  ...
  Blob<unsigned int> channel_rand_vec_;
  /// the probability @f$ p @f$ of dropping any input
  Dtype threshold_;
  Dtype channel_threshold_;
  /// the scale for undropped inputs at train time @f$ 1 / (1 - p) @f$
  Dtype scale_;
  unsigned int uint_thres_;
  unsigned int channel_uint_thres_;
};
...
class ReLULayer : public NeuronLayer<Dtype> {
 public:
  ...
  virtual void Backward_cpu(int N, const Dtype* top_data,const Dtype* top_diff, Dtype* bottom_diff);   
  virtual void Forward_gpu(int N, const Dtype* bottom, Dtype* top);
  virtual void Backward_gpu(int N, const Dtype* top_data,const Dtype* top_diff, Dtype* bottom_diff); 
  */
 protected:
  /**
   * @param bottom input Blob vector (length 1)
