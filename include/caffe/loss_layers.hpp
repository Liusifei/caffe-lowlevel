...
template <typename Dtype>
class BalanceLossLayer : public LossLayer<Dtype> {
 public:
  explicit BalanceLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "BalanceLoss"; }
...
template <typename Dtype>
class ManipulateLossLayer : public LossLayer<Dtype> {
 public:
